  ...
  <nav :class="{ 'h-home-nav': $route.name === 'home' }">
    <router-link to="/">
      <button
        ...
        <div class="absolute top-0 left-0 w-full h-full opacity-50 flex items-center justify-center">
          <font-awesome-icon class="w-[85%] h-44" icon="home"></font-awesome-icon>
        </div>
        <div class="text-3xl skew-x-12 stroke-black dark:mix-blend-plus-lighter">Home</div>
      </button>
    </router-link>
    ...
    <router-link to="/luna">
      <button
        class="bg-slate-800 bg-opacity-40 transition hover:scale-105 border border-solid border-slate-950 w-full h-full drop-shadow-lg -skew-x-12 rounded-2xl cursor-pointer overflow-hidden no-select">
        <div class="absolute top-0 left-0 w-full h-full opacity-50">
          <img src="@/assets/luna.png" alt="Luna" class="w-full h-home-nav object-cover no-select" />
        </div>
        <div class="text-3xl skew-x-12 dark:mix-blend-plus-lighter mix-blend-darken">Luna</div>
      </button>
    </router-link>
  </nav>
</template>
